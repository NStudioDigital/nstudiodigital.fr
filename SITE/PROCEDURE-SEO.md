...
### Core Web Vitals — Seuils cibles Google

| Métrique | Signification | Seuil "Bon" |
|----------|---------------|-------------|
| **LCP** (Largest Contentful Paint) | Vitesse d'affichage du contenu principal | < 2,5 s |
| **INP** (Interaction to Next Paint) | Réactivité aux clics/tap | < 200 ms |
| **CLS** (Cumulative Layout Shift) | Stabilité visuelle (pas de saut de mise en page) | < 0,1 |

> INP a remplacé FID (First Input Delay) depuis mars 2024. Ne pas confondre dans PageSpeed Insights.

**Causes fréquentes de mauvais scores :**
- LCP élevé → image hero trop lourde ou pas preloadée, CSS bloquant
- INP élevé → JavaScript tiers (chatbots, analytics lourds), animations bloquantes
- CLS élevé → images sans `width`/`height`, polices web non préchargées, éléments injectés dynamiquement

**Outil de mesure :** [PageSpeed Insights](https://pagespeed.web.dev/) — tester en mobile ET desktop. Viser le vert sur les 3 métriques avant livraison.
...
- [ ] Core Web Vitals : LCP < 2,5s · INP < 200ms · CLS < 0,1 (PageSpeed Insights, mobile + desktop)
...
## 12. AI OVERVIEWS — OPTIMISER POUR LES RÉSULTATS IA DE GOOGLE

Depuis 2024, Google affiche des **résumés générés par IA** (AI Overviews) en tête des résultats sur les requêtes informatives. Apparaître dans ces encadrés augmente fortement la visibilité même sans clic.

### Pourquoi ça compte pour les clients locaux

Les requêtes du type "meilleur coiffeur curly Lyon", "que faire avant un head spa" ou "sophrologie pour enfants Lyon" déclenchent des AI Overviews. Un site bien structuré peut y figurer même avec peu de notoriété.

### Principes à appliquer dès la rédaction

- **Répondre directement aux questions** : commencer les sections par la réponse, puis développer — Google extrait les premières phrases
- **Format FAQ** : chaque question/réponse courte et autonome (une idée = un bloc)
- **Phrases courtes et sans jargon** : les AI Overviews privilégient le contenu clair et factuel
- **Données concrètes** : mentionner durées, prix, nombres de séances, zones géographiques précises
- **Ton affirmatif** : éviter "on pourrait dire que…" — préférer "la séance dure 60 minutes"

### E-E-A-T — Signaux de confiance à renforcer

Google valorise les contenus qui montrent **Expérience, Expertise, Autorité, Fiabilité** :

- Mentionner le nom et la formation du praticien (page À propos)
- Ajouter des témoignages clients avec prénom et service concerné
- Citer des chiffres concrets (années d'expérience, nombre de clients, certifications)
- Afficher clairement les coordonnées, SIRET, et mentions légales
- Lier vers des sources reconnues quand pertinent (ex. article de blog citant un organisme de santé)

### Structure de contenu recommandée pour les pages services

```
H2 : [Service] — Questions fréquentes
  H3 : Qu'est-ce que [service] ?
  → Réponse directe en 2-3 phrases

  H3 : Combien de temps dure une séance ?
  → "Une séance dure [X] minutes."

  H3 : Quel est le tarif ?
  → "Le tarif est de [X]€."

  H3 : À qui s'adresse [service] ?
  → Réponse ciblée (profils, contre-indications si pertinent)
```

### Ce qu'il ne faut PAS faire

- Rédiger des blocs de texte denses sans structure visible
- Utiliser des titres vagues ("Nos services", "En savoir plus")
- Surcharger de mots-clés — les AI Overviews valorisent la pertinence, pas la densité
- Ignorer le mobile : Google indexe en mobile-first, les AI Overviews aussi

---

---

## 13. JOURNAL DES MISES À JOUR — NSTUDIODIGITAL.FR

### 23 juin 2026 — Optimisation SEO article Planity vs Treatwell + guides client

**Mots-clés dans le corps du texte**
- Insertion des 4 expressions cibles dans `planity-vs-treatwell-outil-reservation.html` : "Planity vs Treatwell" (intro), "Planity ou Treatwell" (section verdict), "alternative à Planity" (section Treatwell), "Treatwell, concurrent de Planity" (section comparatif)
- Enrichissement : +~185 mots répartis dans les sections existantes (~850 mots total)
- `dateModified` JSON-LD mis à jour à `2026-06-23`

**Guides client**
- Création de `fichiers-html/client-guide-search-console.html` : guide 4 étapes — accès GSC, rapport Performances, indexation, alertes prestataire — style identique au guide Google Business, `noindex`

**Configuration repo**
- Création de `SITE/CLAUDE.md` avec instructions permanentes (stack HTML/CSS pur, Vercel auto-deploy, toujours pusher sur `main`)
- Retrait de la règle `CLAUDE.md` dans `.gitignore`

---

### 21 juin 2026 — Audit pré-SEO : réalisations, sitemap, images

**Audit section Réalisations (index.html)**
- Carte Névé Institut déjà présente et correctement intégrée : format `real-card` / `real-card-overlay` identique aux cartes Ko Spa et Studio Mova, image `neve_soin_visage.webp`, tag "Institut de soin · Lyon 6e"
- Aucune correction nécessaire

**Audit sitemap.xml**
- `neve-institut.html` déjà présent dans la section "Pages portfolio" avec `priority 0.6` et `changefreq yearly`
- Aucune correction nécessaire

**Sliders avant/après — état**
- Aucun composant ou section de slider interactif avant/après (drag, compare, ImageCompare…) n'existe dans le repo à ce jour
- Ni code actif, ni section commentée/désactivée sur ko-spa.html, studio-mova.html ou neve-institut.html
- À implémenter en entier dès réception de vraies photos clients

**Doublons images — nommage**
- Quasi-doublon identifié dans `PHOTOS/KO SPA/` : `kospa-headspa.webp` (79 Ko, avec tiret) et `kospaheadspa.webp` (161 Ko, sans tiret)
- Les deux fichiers ont des tailles et contenus distincts, utilisés à deux endroits différents dans ko-spa.html — à conserver
- Règle de rappel : la convention "sans tiret" (section 5) s'applique aux nouvelles images ; les fichiers existants non-dupliqués en contenu restent en place

---

### 18 juin 2026 — Ajout Core Web Vitals et AI Overviews

**Performance**
- Ajout d'une sous-section "Core Web Vitals" dans la section 5 avec les seuils cibles Google (LCP < 2,5s, INP < 200ms, CLS < 0,1) et les causes fréquentes de mauvais scores
- Remplacement de "Temps de chargement < 3s" par la checklist Core Web Vitals dans la section 8
- Note sur le remplacement de FID par INP depuis mars 2024

**AI Overviews**
- Ajout de la section 12 dédiée à l'optimisation pour les résultats IA de Google
- Inclut : principes de rédaction, signaux E-E-A-T, structure de contenu recommandée, erreurs à éviter
...
## 14. SUIVI SEO MENSUEL

Pour les sites en production, effectuer un suivi mensuel via Google Search Console (search.google.com/search-console).

### Rapport Performances

- Ouvrir **Performances** dans le menu gauche
- Vérifier les chiffres sur les **28 derniers jours** : clics, impressions, position moyenne
- Trier les requêtes par **position** pour identifier celles entre 6 et 15 — ce sont les requêtes proches de la page 1, avec le meilleur potentiel d'amélioration rapide
- Une légère baisse ponctuelle est normale ; signaler uniquement si la chute dépasse 30% sur deux semaines consécutives

### Rapport Indexation

- Aller dans **Indexation > Pages**
- Vérifier que toutes les pages principales apparaissent dans "Pages indexées"
- Les statuts **"Page avec redirection"** et **"Autre page avec balise canonique correcte"** sont normaux — pas des erreurs à corriger
- Les pages légales en `noindex` ne doivent pas apparaître dans les pages indexées — comportement attendu

### À savoir

- Les données Search Console deviennent **exploitables à partir de 2 à 3 mois d'ancienneté** du site — en dessous, les chiffres sont trop parcellaires pour des conclusions fiables
- Fréquence recommandée pour un site vitrine local : une vérification par mois suffit

---

## 15. OPTIMISATION ARTICLES BLOG

### Insertion des mots-clés dans le corps du texte

- Les mots-clés cibles doivent apparaître **dans les paragraphes rédigés**, pas seulement dans le `<title>`, le H1 et le JSON-LD
- Insérer chaque expression longue traîne dans la section thématiquement cohérente — une occurrence naturelle par expression suffit
- Éviter le bourrage : Google détecte les densités anormales — fluidité de lecture avant tout

### Volume de contenu

- Viser **850 à 900 mots minimum** pour les articles comparatifs ou informatifs
- Pour enrichir sans créer de nouvelles sections : allonger les introductions de section, ajouter des paragraphes de synthèse, étoffer les callouts existants

### JSON-LD BlogPosting — dateModified

- Mettre à jour `"dateModified"` dans le JSON-LD BlogPosting à **chaque modification de l'article**, même mineure
- Format : `"AAAA-MM-JJ"` — ex. `"2026-06-23"`
- Le `"datePublished"` ne doit jamais être modifié rétroactivement

### Potentiel de la longue traîne

- Les requêtes longue traîne bien ciblées (ex. "planity vs treatwell") peuvent atteindre **la première page Google en quelques semaines** sur un marché peu concurrentiel
- Prioriser ces expressions dans les premiers articles pour obtenir des signaux de trafic rapidement

---

## 16. LIVRABLES CLIENT

### Guides client disponibles

| Fichier | Sujet | Distribution |
|---------|-------|--------------|
| `client-guide-google-business.html` | Créer et optimiser la fiche Google Business | `noindex` — envoi par email à la livraison |
| `client-guide-search-console.html` | Lire Google Search Console, identifier les alertes | `noindex` — envoi par email à la livraison |

### Règles de création des guides client

- Toujours en `noindex, nofollow` — documents internes, non destinés à l'indexation
- Ton : simple, accessible, zéro jargon — lectorat : coiffeurs et praticiens bien-être
- Style visuel : identique au guide Google Business existant (CSS de référence)
- Envoi systématique par email à la livraison ou à la demande du client

---

## 17. CONFIGURATION REPO

### CLAUDE.md

- Un fichier `CLAUDE.md` est présent à la racine du repo (`SITE/CLAUDE.md`) — suivi par Git depuis le 23 juin 2026
- Il contient les instructions permanentes pour les sessions de travail :
  - Stack HTML/CSS pur — jamais WordPress, CMS, frameworks ou outils no-code
  - Tous les fichiers sont des `.html` et `.css` natifs
  - Ne jamais suggérer de plugins, thèmes ou solutions CMS
  - Déploiement automatique sur **Vercel** depuis la branche `main` de `NStudioDigital/nstudiodigital.fr`
  - Toujours pusher les modifications sur `main` après chaque changement
- La règle `CLAUDE.md` a été retirée du `.gitignore` le 23 juin 2026 (la règle `CLAUDE/` est conservée)

---

*Procédure N/ Studio Digital — Version 1.5 — Juin 2026*
